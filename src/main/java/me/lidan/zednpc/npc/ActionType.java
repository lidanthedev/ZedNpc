...
public enum ActionType {
    ...
    public static void helpActions(Audience audience) {
        audience.sendMessage(MiniMessageUtils.miniMessage("<green>Available Actions: <yellow><bold><hover:show_text:'<gold>Make player run command. example: <yellow>CMD warp pvp'>CMD</hover> <hover:show_text:'<gold>Make console run command. example: <yellow>CONSOLE give * diamond'>CONSOLE</hover> <hover:show_text:'<gold>Make player send chat message. example: <yellow>CHAT come spawn'>CHAT</hover> <hover:show_text:'<gold>Send message in chat. example: <yellow>MESSAGE this is the server'>MESSAGE</hover> <hover:show_text:'<gold>Send player to other server (proxy only). example: <yellow>SERVER lobby'>SERVER</hover> <hover:show_text:'<gold>Hover over actions to learn more'><gold><bold>HOVER FOR HELP</hover>"));
